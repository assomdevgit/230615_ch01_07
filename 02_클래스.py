...
store1 = Store(location="신대방삼거리점")
print(store1.title, store1.menu, store1.location)
store3 = Store(location="방배점")
...
class Store:
    ...
    def __init__(self, name="붕어빵 스토어"):
        ...
        print(f"(3)self.name : {Store.name}")
...
class MetaMetaHuman(MetaHuman):
    def run(self):
        #super().run() # MetaHuman, Human 호출
        #super(MetaMetaHuman, self).run() # MetaHuman, Human
        super(MetaHuman, self).run() # Human
